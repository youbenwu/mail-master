...
import com.ys.mail.util.BaiduUtil;
...
public class UmsPartnerReviewServiceImpl extends ServiceImpl<UmsPartnerReviewMapper, UmsPartnerReview> implements UmsPartnerReviewService {
    ...
    public CommonResult<String> apply(UmsPartnerReviewDTO req) {
        ...
        List<UmsPartnerReview> umsPartnerReviews = partnerReviewMapper.selectList(Wrappers.<UmsPartnerReview>lambdaQuery()
        ...
        // 3. 获取地址的经纬度
        Map<String, Double> map = BaiduUtil.getLngAndLat(req.getProvince() + req.getCity() + req.getRegion() + req.getAddress());
        Double lng = map.get("lng");
        Double lat = map.get("lat");
        partnerReview.setUserId(UserUtil.getCurrentUser().getUserId());
        partnerReview.setLatitude(lng);
        partnerReview.setLongitude(lat);
    ...
    public CommonResult<Map<String, Object>> judge() {
        ...
        UmsPartner umsPartner = partnerMapper.selectOne(Wrappers.<UmsPartner>lambdaQuery().eq(UmsPartner::getUserId, userId));
        if (ObjectUtils.isNotEmpty(umsPartner)) {
            map.put("isPartner", true);
        } else {
            map.put("isPartner", false);
            UmsPartnerReview umsPartnerReview = partnerReviewMapper.selectOne(Wrappers.<UmsPartnerReview>lambdaQuery().eq(UmsPartnerReview::getUserId, userId));
