...
public class EveryMonthJob {
    ...
    public void userPlain() {
        List<UmsUser> umsUserList = umsUserMapper.selectList(Wrappers.<UmsUser>lambdaQuery().eq(UmsUser::getDeleted, 0));
    ...
    public void masterPlain() {
        List<UmsUserInvite> umsUserInviteList = umsUserInviteMapper.selectList(Wrappers.<UmsUserInvite>lambdaQuery().groupBy(UmsUserInvite::getParentId));
        if (ObjectUtils.isNotEmpty(umsUserInviteList)) {
            List<Long> ids = umsUserInviteList.stream().map(UmsUserInvite::getParentId).distinct().collect(Collectors.toList());
            for (Long id : ids) {
    ...
    public void masterPlain2(String parentId) {
        ...
        List<UmsUserInvite> umsUserInviteList = umsUserInviteMapper.selectList(Wrappers.<UmsUserInvite>lambdaQuery().eq(UmsUserInvite::getParentId, parentId));
        List<Long> ids = umsUserInviteList.stream().map(UmsUserInvite::getUserId).collect(Collectors.toList());
        // 上个月活跃用户数
        Integer activeCount = umsUserMapper.selectCount(Wrappers.<UmsUser>lambdaQuery().in(UmsUser::getUserId, ids).eq(UmsUser::getIsActive, 1));
        ...
        if (activeCount >= 20 + (20 * level)) {
            ...
            nextGroupMaster.setRatio(BigDecimal.valueOf(groupMater.getRatio()).add(BigDecimal.valueOf(0.0005D)).doubleValue());
    ...
    public void userPlain2(String userId) {
        ...
        Integer first = Integer.valueOf(localDate.with(TemporalAdjusters.firstDayOfMonth()).format(DateTimeFormatter.ofPattern("yyyyMMdd")));
        Integer last = Integer.valueOf(localDate.with(TemporalAdjusters.lastDayOfMonth()).format(DateTimeFormatter.ofPattern("yyyyMMdd")));
        ...
        if (ObjectUtils.isNotEmpty(gameDate) && gameDate.stream().filter(obj -> obj >= first && obj <= last).distinct().collect(Collectors.toList()).size() >= 20) {
            // 上月佣金
