...
public class MonitorServiceImpl implements MonitorService {
    ...
    public Map<String, Object> getServers(boolean refresh) {
        ...
        // 获取缓存过期时间
        Long expire = redisConfig.getExpire().getMinute();
        ...
        return Optional.ofNullable(o)
                ...
                .orElseGet(() -> {
                    Map<String, Object> resultMap = new LinkedHashMap<>(7);
                    try {
                        ...
                        // 当期时间
                        ResultUtil.setUnifyParam(resultMap, expire.intValue());
                        // 设置到缓存
                        if (!refresh) {
                            ResultUtil.setUnifyParam(resultMap, expire.intValue());
                            redisService.set(fullKey, resultMap, expire);
                        } else {
    ...
    private Map<String, Object> getDiskInfo(OperatingSystem os) {
        ...
        diskInfo.put("total", total > 0 ? FileTool.getSize(total) : "?");
        diskInfo.put("available", FileTool.getSize(available));
        diskInfo.put("used", FileTool.getSize(used));
        if (total != 0) {
    ...
    private Map<String, Object> getSystemInfo(OperatingSystem os) {
        ...
        systemInfo.put("startTime", time);
        systemInfo.put("runtime", formatBetween);
